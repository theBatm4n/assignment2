...
public class BigramFrequencyPairs extends Configured implements Tool {
	...
	private static class MyMapper extends
		...
		public void map(LongWritable key, Text value, Context context)
			...
			for(int i= 0; i < words.length - 1; i++){
				// removing punctutation and converting to lower case
	...
	private static class MyReducer extends
		...
		public void reduce(PairOfStrings key, Iterable<IntWritable> values,
			...
			// If we're starting with a new left word, reset current total
			...
			if(key.getRightElement().equals("*")){
				...
				currentTotal = (float) sum;

				PairOfStrings totalKey = new PairOfStrings(key.getLeftElement(), "");
				VALUE.set(currentTotal);
				context.write(totalKey, VALUE);
			}
			else{
				// Process bigram only if we have the total
				if (currentTotal > 0) {
					int sum = 0;
					for (IntWritable value : values) {
						sum += value.get();
